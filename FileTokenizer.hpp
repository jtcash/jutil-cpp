...
//     // Open the file for reading, in binary mode, and at the end to get the filesize 
//     std::ifstream ifs(fileName, std::ios::in | std::ios::binary | std::ios::ate);
//     echo(ifs.good());
//     // Query the filesize, will be -1 if file does not exist
...
struct converter<double>{
  ...
  converter(std::string_view src){
    ...
    char * end;
    auto res = std::strtod(buf.data(), &end);
    if(end == buf.data())
      result = std::nullopt;
    ...
    // std::string src2(src);
    // char * end;
    // auto res = std::strtod(src2.data(), &end);
    // if(end == src2.data())
    //   result = std::nullopt;
    // result = std::make_optional(res);
  }

  constexpr operator std::optional<double>() const{
    return result;
  }
};
...
class FileReader{
  ...
  template<class T>
  decltype(auto) error_message(T&& t, std::string_view status_message){
    std::cerr << "ERROR: " << status_message << " for file \"" << filename << '"' << std::endl;
    return std::forward<T>(t);
  }
public:
  // std::array<std::array<char, buffer_size>, 2> buffer;
  ...
  FileReader(std::string filename) : filename{std::move(filename)}, file{} { }
  ...
  // FileReader(FileReader&& that) noexcept = default;
  //: filename(std::move(that.filename)), file(std::move(that.file)) {  }
...
struct Tokenizer : public TokenizerBase{
  ...
  std::optional<AsType> getToken(Args&&... args){
    ...
    // double val;
    // std::cerr << '.' ;
    return converter<AsType>(tok);
    // auto res = std::from_chars(tok.begin(), tok.end(), val);

    // if(res.) 
    // const char * be = tok.begin();
    // const char * en = tok.end();


    // // auto res = std::from_chars(be, en, val);
    
    // // if(auto [p, ec] = std::from_chars(be, en, val); ec == std::errc())
    // // // if(auto [p, ec] = std::from_chars(tok.data(), tok.data()+tok.size(), val); ec == std::errc())
    // //     return std::nullopt;
...
inline Tokenizer<FileReader, BlockSize> makeFileTokenizer(std::string filename){
  return makeTokenizer<BlockSize>(FileReader(std::move(filename), true));
}
